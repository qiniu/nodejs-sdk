...
export function updateProgress(evt, index, progress, totalSize) {
  ...
  if (evt.lengthComputable) {
    if(index!="no"){
      let progress_unit = progress[index];
      progress_unit.total = evt.total;
      newLoad = evt.loaded - progress_unit.loaded;
      progress_unit.loaded = evt.loaded;
      progress_unit.percent = Math.round(evt.loaded / evt.total * 100);
    }else{
      newLoad = evt.loaded - progress_total.loaded;
    }
  } else {
    console.log("文件内容为空");
    return false;
  }
  progress_total.loaded = progress_total.loaded + newLoad;
  let total_percent = Math.round(progress_total.loaded / totalSize * 100);
  progress_total.percent = total_percent > 100 ? 100 : total_percent;
  return progress;
}
