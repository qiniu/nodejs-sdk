...
Service.prototype.mkbucket = function(bucketname, onret) {
...
Service.prototype.upload = function(upToken, key, mimeType, filename, stream, onret) {
	...
	**/
	...
	this.conn.callWith(upToken, form, onret);
};
...
Service.prototype.setSeparator = function(sep, onret){
	var sepBuffer = new Buffer(sep);
	sep = sepBuffer.toString('base64');
...
Service.prototype.setStyle = function(name, style, onret){
	...
	name = nameBuffer.toString('base64');

	var styleBuffer = new Buffer(style);
	style = styleBuffer.toString('base64');
