...
Service.prototype.list = function(marker, limit, prefix, onret) {
  /*
   * func List(marker, limit, prefix, onret) => Entry[], marker
   * 列出某个存储空间（bucket）文件
   */
  var url = config.RSF_HOST + '/list';
  url += '?bucket=' + this.bucket;
  url += '&marker=' + (marker || '');
  url += '&limit=' + (limit || '');
  url += '&prefix=' + (prefix || '');
  this.conn.callWith(url, null, onret);
